/*
 ...
 - Insert x, где 0 ≤ x ≤ 10 — целое число;
 - ExtractMax.
...
extension Solution {
    ...
    @_optimize(speed)
    func priorityQueue(handle actions: [String]) -> [Int] {
        var result: [Int] = []
        var heap = Heap<Int>(sort: >)

        for action in actions {
            if action == "ExtractMax" {
                let root = heap.removeRoot() ?? 0
                result.append(root)
            }
            
            if action.contains("Insert") {
                if let value = action.split(separator: " ").last,
                   let int = Int(value) {
                    heap.insert(int)
                }
            }
    ...
    func priorityQueueSolution() {
        ...
        let result = priorityQueue(handle: actions)
